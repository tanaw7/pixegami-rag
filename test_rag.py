...
EVAL_PROMPT = """
You are an expert evaluator. Compare the expected response with the actual response and determine if they convey the same meaning or key information, even if worded differently.
...
def evaluate_response(question: str, expected_response: str, actual_response: str, retries: int = 3) -> bool:
    """Evaluate if actual_response is semantically equivalent to expected_response using LLM."""
    model = ChatOpenAI(api_key=API_KEY, model="gpt-3.5-turbo", temperature=0.0)
    ...
    for _ in range(retries):
        ...
        if result in ("true", "false"):
            return result == "true"
    raise ValueError(f"LLM evaluation failed after {retries} attempts. Last result: {result}")
...
def chroma_db(tmp_path_factory):
    ...
    monopoly_doc = Document(
        page_content="Each player starts with $1500 in Monopoly. Properties can be bought when landing on an unowned property at its printed price.",
